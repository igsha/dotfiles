Config {
       ...
       , commands = [ Run Memory ["-t","<icon=.xmonad/xbm8x8/mem.xbm/> <usedratio>%", "-w", "2"] 10
                    ...
                    , Run Network "wlan0" ["-t", "<icon=.xmonad/xbm8x8/wifi_02.xbm/> <rx>KB:<tx>KB", "-L","0","-H","32","-w", "3", "--normal","green","--high","red"] 30
                    , Run Kbd [("us", "US"), ("ru", "RU")]
       ...
       , sepChar = "%"
       , alignSep = "}{"
       , template = "%trayer%%StdinReader% }{%locks% | %kbd% | <action=pavucontrol>%default:Master%</action> | %coretemp% | %multicpu% | %memory% | %battery% | %eth0% %wlan0% | %UUEE% | <action=xclock -update 1><fc=#ee9a00>%date%</fc></action>"
       }
